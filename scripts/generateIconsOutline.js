...
const generateIconComponent = async (filePath, iconName) => {
  ...
  const cleanedJsxCode = jsxCode
    ...
    .replace(
      ...
      (match, p1, p2, p3) => `<${p1}${p2} stroke={color || 'currentColor'}`
    );
...
generateIcons()
  .then(() => console.log("Outline icons generated successfully"))
  .catch((err) => console.error("Error generating outline icons:", err));
